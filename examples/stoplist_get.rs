...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.get_stoplist().await?;

    println!(
        "status: {:?}, status_code: {:?}, status_text: {:?}, stoplist: {:?}",
        response.status, response.status_code, response.status_text, response.stoplist
    );
