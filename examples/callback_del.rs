...
use smsru::{Auth, CallbackUrl, RemoveCallback, SmsRuClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = RemoveCallback::new(CallbackUrl::new(callback_url)?);
    let response = client.remove_callback(request).await?;
